...
Lara CLI supports multiple file formats (JSON, PO, etc.). Each format is configured separately under the `files` section. See [Supported Formats](./formats.md) for a complete list and format-specific documentation like [PO Files Guide](./po-files.md).
